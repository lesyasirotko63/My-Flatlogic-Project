...
import actions from 'actions/reviews/reviewsListActions';
...
const ReviewsTable = ({ classes }) => {
  ...
  const count = useSelector((store) => store.reviews.list.count);
  const modalOpen = useSelector((store) => store.reviews.list.modalOpen);
  const rows = useSelector((store) => store.reviews.list.rows);
  const idToDelete = useSelector((store) => store.reviews.list.idToDelete);
      ...
      <div>
        ...
        >
          <MenuItem
            classes={classes}
            onClick={() => {
              history.push(`/admin/reviews/${cell.id}/edit`);
              handleClose();
            }}
          >
            <EditIcon />
  ...
  return (
    <div>
      <Widget title={<h4>Reviews</h4>} disableWidgetMenu>
