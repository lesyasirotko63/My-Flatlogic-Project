...
const CategoriesTable = ({ classes }) => {
  ...
  const count = useSelector((store) => store.categories.list.count);
  const modalOpen = useSelector((store) => store.categories.list.modalOpen);
  const rows = useSelector((store) => store.categories.list.rows);
  const idToDelete = useSelector((store) => store.categories.list.idToDelete);
      ...
      <div>
        ...
        >
          <MenuItem
            classes={classes}
            onClick={() => {
              history.push(`/admin/categories/${cell.id}/edit`);
              handleClose();
            }}
          >
            <EditIcon />
  ...
  return (
    <div>
      <Widget title={<h4>Categories</h4>} disableWidgetMenu>
