...
import actions from 'actions/promocodes/promocodesListActions';
...
const PromocodesTable = ({ classes }) => {
  ...
  const count = useSelector((store) => store.promocodes.list.count);
  const modalOpen = useSelector((store) => store.promocodes.list.modalOpen);
  const rows = useSelector((store) => store.promocodes.list.rows);
  const idToDelete = useSelector((store) => store.promocodes.list.idToDelete);
      ...
      <div>
        ...
        >
          <MenuItem
            classes={classes}
            onClick={() => {
              history.push(`/admin/promocodes/${cell.id}/edit`);
              handleClose();
            }}
          >
            <EditIcon />
  ...
  return (
    <div>
      <Widget title={<h4>Promocodes</h4>} disableWidgetMenu>
          ...
          <a
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Promocodes'
                : 'http://localhost:8080/api-docs/#/Promocodes'
            }
          >
            API documentation for promocodes
          </a>
        </div>
      </Widget>
...
export default withStyles(styles)(PromocodesTable);
