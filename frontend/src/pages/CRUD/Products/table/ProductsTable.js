...
const ProductsTable = ({ classes }) => {
  ...
  const count = useSelector((store) => store.products.list.count);
  const modalOpen = useSelector((store) => store.products.list.modalOpen);
  const rows = useSelector((store) => store.products.list.rows);
  const idToDelete = useSelector((store) => store.products.list.idToDelete);
      ...
      <div>
        ...
        >
          <MenuItem
            classes={classes}
            onClick={() => {
              history.push(`/admin/products/${cell.id}/edit`);
              handleClose();
            }}
          >
            <EditIcon />
  ...
  return (
    <div>
      <Widget title={<h4>Products</h4>} disableWidgetMenu>
